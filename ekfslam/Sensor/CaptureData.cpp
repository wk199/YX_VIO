...
//    memset(Imu_char_data,0,sizeof(ImuCapture));
    ...
    Imu_num=10;
    Imu_data.DataNum=10;
    cout<<"########"<<Imu_data.IMG_Frame_Buffer.size();
    for(k=0;k<Imu_data.IMG_Frame_Buffer.size();k++)
    {
        cout<<Imu_data.IMG_Frame_Buffer[k].ImgTimeStamp<<"*******";
    }
    cout<<endl;
//    memset(Imu_data.ImuTimeStamp+10,0,sizeof(Imu_data.ImuTimeStamp));
//    memset(Imu_data.ImuValue+10,0,sizeof(Imu_data.ImuValue));
    Imu_data.IMG_Frame_Buffer.clear();
...
//                cv::waitKey(1);
                p_Mutex.Lock();
                Imu_data.ImgTimeStamp=ImgTimestamp;
                IMG_Frame_data.ImgTimeStamp=ImgTimestamp;
                memcpy(Imu_data.LeftImgBuffer,m_pOutDataLeft,480*640);
                memcpy(Imu_data.RightImgBuffer,m_pOutDataRight,480*640);
                memcpy(IMG_Frame_data.LeftImgBuffer,m_pOutDataLeft,480*640);
                memcpy(IMG_Frame_data.RightImgBuffer,m_pOutDataRight,480*640);
                Imu_data.IMG_Frame_Buffer.push_back(IMG_Frame_data);
                p_Mutex.UnLock();
