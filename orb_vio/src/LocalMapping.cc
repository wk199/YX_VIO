...
LocalMapping::LocalMapping(Map *pMap):
    mbResetRequested(false), mpMap(pMap),  mbAbortBA(false), mbStopped(false), mbStopRequested(false), mbAcceptKeyFrames(true),optUseIMU(false),IfoptStop(NULL)
{
}
...
void LocalMapping::SetIMU(IMUMeasurementList* _imufactor)
{
	mpIMU = _imufactor;
}
...
void LocalMapping::Run()
{
    ...
    while(true)//ros::ok())
    {
        ...
        {            
            ...
            mbAbortBA = false;
            cout << "In localmapping with Frame " << mpCurrentKeyFrame->mnFrameId << ", mbAbortBA = " << mbAbortBA << std::endl;
            ...
            if(!CheckNewKeyFrames() && !stopRequested())
            {
	            ...
	            //Optimizer::LocalBundleAdjustmentUseISAM(mpCurrentKeyFrame , mpIMU);
                //Optimizer::LocalBundleAdjustmentUseISAM2(mpCurrentKeyFrame , mpMap);
                if(optUseIMU){
                    //Optimizer::LocalBundleAdjustmentUseIMU(mpCurrentKeyFrame , mpIMU, &mbAbortBA);
                    Optimizer::LocalBundleAdjustmentUseIMUTemporal(mpCurrentKeyFrame, mpIMU, mpMap, IfoptStop, &mbAbortBA);            
                }
                else{
                    Optimizer::LocalBundleAdjustment(mpCurrentKeyFrame,&mbAbortBA);
                }
                //Optimizer::LocalBundleAdjustmentGTSAM(mpCurrentKeyFrame);
                //Optimizer::LocalBundleAdjustmentGTSAM2(mpCurrentKeyFrame , mpIMU);

                //Check redundant local Keyframes
                if(*IfoptStop == false)
                    KeyFrameCulling();
                mpCurrentKeyFrame->updateAfterBA = true;
...
void LocalMapping::SetOptUseIMU(bool flag){
    optUseIMU = flag;
}

void LocalMapping::SetOptStop(bool *flag){
    IfoptStop = flag;
}
...
void LocalMapping::KeyFrameCulling()
{
    ...
    if(mpCurrentKeyFrame->mnId >frameUseForBA+1)
    {
    for(vector<KeyFrame*>::iterator vit=vpLocalKeyFrames.begin(), vend=vpLocalKeyFrames.end(); vit!=vend; vit++)
    {
        KeyFrame* pKF = *vit;
        if(pKF->mnId==0 || pKF->mnId >= mpCurrentKeyFrame->mnId - frameUseForBA)
            continue;
        ...
        {
            pKF->SetBadFlag();
            //cout << "culling KeyFrame id = " << pKF->mnId << endl;
        }
