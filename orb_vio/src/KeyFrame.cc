...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    mnLoopQuery(0), mnRelocQuery(0),fx(F.fx), fy(F.fy), cx(F.cx), cy(F.cy), mBowVec(F.mBowVec),updateAfterBA(false),
    ...
    mvInvLevelSigma2(F.mvInvLevelSigma2), mpMap(pMap) , mnBALocalForISAM2(0) , mnBAFixedForISAM2(0)
{
    mnId=nNextId++;
    ...
    for(int i=0;i<3;i++)
    {
        mSpeedAndBias.vel[i]=0;
        mSpeedAndBias.accbias[i]=0;
        mSpeedAndBias.gyrobias[i]=0;
    }
    SetPose(F.mTcw);    
}
...
cv::Point2f KeyFrame::projectMapPoint(MapPoint* mp){
    
    cv::Mat mp_pos = mp->GetWorldPos();
    //cout << "mp_pos = " << mp_pos.t() << endl;
    cv::Mat R(3,3,CV_32F);
    cv::Mat T(3,1,CV_32F);
    R = Tcw.rowRange(0,3).colRange(0,3);
    T = Tcw.rowRange(0,3).col(3);
    //cout << "T = " << T.t()<<endl;
    cv::Mat mp_pos_in_KF = R * mp_pos + T;
    //cout << "mp_pos_in_KF = " << mp_pos_in_KF.t() << endl;
    float invZ1 = 1.0 / mp_pos_in_KF.at<float>(2);
    float imx = fx * mp_pos_in_KF.at<float>(0) * invZ1 + cx;
    float imy = fy * mp_pos_in_KF.at<float>(1) * invZ1 + cy;
    
    return cv::Point2f(imx , imy);
