...
int ORBmatcher::SearchByProjection(Frame &CurrentFrame, KeyFrame* pKF, float th)
{
    int nmatches = 0;
    ...
    std::vector<MapPoint*> mMapPoints = pKF->GetMapPointMatches();

    for(size_t i=0, iend=mMapPoints.size(); i<iend; i++)
    {
        MapPoint* pMP = mMapPoints[i];
        ...
        if(pMP)
        {
               ...
               int nPredictedOctave = pKF->mvKeys[i].octave;
                ...
                cv::Mat dMP = pKF->mDescriptors.row(i);
