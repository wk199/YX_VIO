...
void Optimizer::BundleAdjustment(const vector<KeyFrame *> &vpKFs, const vector<MapPoint *> &vpMP, int nIterations, bool* pbStopFlag)
{
    ...
    //Keyframes
    //int curKFId = pKF->mnId;
    std::ofstream fout;
	fout.open("result_ba.txt");
    for(size_t i=0, iend=vpKFs.size(); i<iend; i++)
    {
        KeyFrame* pKF = vpKFs[i];
        ...
        fout << "Camera " << pKF->mnId << " = " << Converter::toCvMat(SE3quat) << std::endl;
    ...
    for(size_t i=0, iend=vpMP.size(); i<iend;i++)
    {
        ...
        fout << "Point " << pMP->mnId << " = " << Converter::toCvMat(vPoint->estimate()) << std::endl;
        
        const map<KeyFrame*,size_t> observations = pMP->GetObservations();
		//fout << "Point "<< pMP->mnId << " = " << pMP->GetWorldPose() << std::endl;
...
{    
    ...
    //cout << "PoseOptimization with inlier number is " << nInitialCorrespondences << ", remain " << nInitialCorrespondences-nBad << endl;
...
{    
    ...
    for(list<KeyFrame*>::iterator lit=lLocalKeyFrames.begin(), lend=lLocalKeyFrames.end(); lit!=lend; lit++)
    {
        ...
        g2o::SE3Quat SE3quat = vSE3->estimate();
        pKFi->SetPose(Converter::toCvMat(SE3quat));
        fout << "Camera " << pKFi->mnId << " = " << Converter::toCvMat(SE3quat) << std::endl;
    }
