...
Tracking::Tracking(ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer,
	...
	mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer),mpMap(pMap),keyframeNum(0),OptStop(false),
...
Tracking::Tracking(ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer,
	...
	mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer),mpMap(pMap),OptStop(false),
...
void Tracking::SetLocalMapper(LocalMapping *pLocalMapper)
{
    ...
    mpLocalMapper->SetOptStop(PointerOptStop);
}

void Tracking::setPointer(bool *Flag)
{
    PointerOptStop = Flag;
}
...
bool Tracking::TrackUseImuPredict()
{
    ...
    if(nmatches <= 50)
    {
        if(!*PointerOptStop && !mpLocalMapper->AcceptKeyFrames())
        {
            *PointerOptStop = true;
        }
